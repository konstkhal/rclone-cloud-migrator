...
- Crash-safe resume: chunk numbering is persisted per source+destination task, so an interrupted run picks back up at the correct next index instead of resetting to `part001` and risking an overwrite of already-completed chunks. If no state file exists yet, it self-heals by probing the destination for the highest existing chunk already there. That persisted number is a destination *name* index, not a position inside the current manifest: a resume subtracts the naming base recorded when the manifest was frozen, and halts rather than guessing if that base is missing.
- Deterministic manifest: the source is listed once per task and the sorted result is frozen to a local state file instead of being re-listed on every resume — a live recursive listing does not guarantee stable ordering across separate calls against a very large folder, which can otherwise shift chunk boundaries between attempts. Freezing also records the naming offset in force at that moment to a companion `.manifest_base__<task>.state` file, which is what makes a later resume able to tell "already archived" from "not yet named". Delete the frozen manifest file to force a fresh scan; the base is re-recorded with it.
- Completion contract: the pipeline may not return success without proving it. Under `purge=yes` it re-lists the source after the purge queue drains and exits non-zero if any file is still there; under `purge=no` it asserts every planned batch was processed. An exit 0 means the source is drained — it may not mean anything weaker.
- Dry-run aware: chunks are still built and locally verified for a realistic preview, but remote push, verification, purge, and buffer flush are skipped. The completion assertion is skipped too.
...
- TAR-CHUNK mode also logs every rclone invocation's own full diagnostic output (manifest scan, FUSE mount, chunk push, async purge delete) to a dedicated timestamped rclone log (`logs/`) — not just this script's own summary line — so a stall or rate-limit error is traceable to its actual cause instead of a generic timeout message.
...
**Current version: 5.8.0** - TAR-CHUNK resume no longer confuses the persisted chunk index with a position inside the frozen manifest. That index names the next tar on the destination; the two coincide only when a manifest is frozen with no naming offset in force, and against `docu_trans_process` they diverged into an 86,975-file / 169.96 GiB gap that the pipeline skipped in one step while reporting success and exiting 0. Freezing now records the offset in force to `state/.manifest_base__<task>.state` and a resume skips `next_chunk_index - base`; a reused manifest with no recorded base halts rather than assuming zero. Adds a completion contract, so the pipeline cannot return success without re-listing the source (`purge=yes`) or asserting the plan was consumed (`purge=no`), on every exit path including the "already completed in a prior run" early return. Ships this repository's first test suite, `tests/resume_arithmetic.bats` (bats-core, 15 cases, `rclone` replaced by a recording double). Live-validated 2026-09-20 by draining that exact gap: 34 chunks, 170.03 GiB, source verified empty.

**v5.7.0** - TAR-CHUNK freezes an `LC_ALL=C`-sorted manifest per task instead of re-scanning the remote on every resume, since a live recursive listing of a very large folder does not guarantee stable ordering across calls. Every TAR-CHUNK rclone invocation (scan, mount, push, purge) now writes to one durable timestamped rclone log rather than losing its diagnostics on failure. Note that 5.7.0's third fix, a position-based resume skip, was itself the defect 5.8.0 corrects.

**v5.6.0** - queuing a drilled-down subfolder no longer removes its top-level parent from the source folder menu (previously queuing `dir1/dir1-1` made every other subfolder of `dir1` unreachable for the rest of the session). The menu-retirement mechanism is replaced by a registry of already-queued source paths that both listing menus filter against, which also closes the inverse gap: the same path can no longer be queued twice. Queuing a parent whose subtree already holds queued tasks now prints the nested task list and asks for confirmation - legitimate when the nested tasks purge first, a double-copy when they don't. Also fixes "Reset and re-configure queue from scratch" leaving the discarded queue's directory-preservation flags behind, misaligning them with tasks queued after the reset.

**v5.5.2** - the remote-lock write-failure warning now includes rclone's actual error text instead of a fixed "(no write access?)" guess. The lock-file write's stderr was previously discarded; it is now captured (newlines squashed to a single line) and appended to the warning as `rclone: <error>`. This surfaced why the Dropbox source-side lock had never been created since v4.3.0: the source account is over its storage quota, so every upload including the tiny lock file is rejected with `insufficient_space`, while deletes and reads keep working (so the copy-and-purge path never showed it). Otherwise unchanged - a failed lock write stays non-fatal, and the destination-side (Google Drive) lock still guards the same-task concurrency case.

**v5.5.1** - RAW mode no longer nests the source's own path under the destination: `rclone copy` now targets exactly the directory picked in the destination prompt, so a folder's contents land there directly instead of underneath a recreated copy of its full source path. TAR and TAR-CHUNK's archive filename / chunks-folder name switched from the full drilled-down path to just the leaf folder name, for the same reason. The RAW purge gate's `rclone check` now runs `--one-way`, since a flattened destination can legitimately hold files that never came from the current source (siblings, prior runs) - purge now correctly gates on "did everything from source land intact," not "does the destination contain nothing else."

**v5.5.0** - all four interactive listing menus (remote selection, destination browsing, source top-level and subfolder drilldown) now paginate instead of dumping the full listing at once, so a folder with hundreds or thousands of entries no longer overflows a detached screen session's scrollback. `n`/`p` page in place, entries stay selectable by absolute index from any page, and the page resets on every drilldown/back/reset.

**v5.4.2** - source folder drilldown's 'b' and 'r' now actually navigate back to the top-level source menu instead of dead-ending at the chosen top-level folder ('b' refused outright, 'r' silently no-opped despite its "reset to remote root" label) - previously a wrong folder pick forced a full restart of the configuration flow.
